...
class EkbkyrgyzdarSupabaseUser extends BaseAuthUser {
  EkbkyrgyzdarSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> ekbkyrgyzdarSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = EkbkyrgyzdarSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
